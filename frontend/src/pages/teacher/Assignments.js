...
const TeacherAssignments = () => {
  ...
  const [classStudentMappings, setClassStudentMappings] = useState([]);
  const [loadingClassStudentMappings, setLoadingClassStudentMappings] = useState(false);
  ...
  // Fetch teacher assignment data on mount
  useEffect(() => {
    fetchAssignments();
    fetchClassStudentMappings();
  }, []);
  ...
  // Fetch teacher's class-student mappings used for student-first selection flow.
  const fetchClassStudentMappings = async () => {
    setLoadingClassStudentMappings(true);
    try {
      const response = await api.get('/teacher/class-students');
      setClassStudentMappings(Array.isArray(response.data) ? response.data : []);
    } catch (error) {
      console.error('Error fetching class-student mappings:', error);
      setClassStudentMappings([]);
      toast.error('Failed to load students');
    } finally {
      setLoadingClassStudentMappings(false);
    }
  };

  const studentOptions = Array.from(
    new Map(
      classStudentMappings.map((row) => [
        row.student_id,
        { id: row.student_id, name: row.student_name }
      ])
    ).values()
  ).sort((a, b) => a.name.localeCompare(b.name));

  const classOptionsForStudent = formData.student_id
    ? Array.from(
        new Map(
          classStudentMappings
            .filter((row) => String(row.student_id) === String(formData.student_id))
            .map((row) => [
              row.class_id,
  ...
  const handleChange = (e) => {
    ...
    if (name === 'student_id') {
      setFormData(prev => ({ ...prev, student_id: value, course_id: '' }));
      return;
    }
  ...
  const validateForm = () => {
    ...
    const isClassValidForStudent = classOptionsForStudent.some(
      (classItem) => String(classItem.id) === String(formData.course_id)
    );

    if (!isClassValidForStudent) {
      toast.error('Selected class does not belong to selected student');
      return false;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      submitData.append('deadline', formData.due_date);
      submitData.append('day_of_week', formData.day_of_week);
      submitData.append('class_id', formData.course_id);
  ...
  return (
      ...
      {/* Assignments Table */}
      <div className="assignments-table-container table-wrapper">
        {loading ? (
              ...
              <div className="form-row">
                <div className="form-group">
                  ...
                  <select
                    ...
                    onChange={handleChange}
                    disabled={loadingClassStudentMappings}
                    required
                  >
                    <option value="">
                      {loadingClassStudentMappings
                        ? 'Loading Students...'
                  ...
                  >
                    <option value="">
                      {!formData.student_id
                        ...
                        : classOptionsForStudent.length === 0
                          ? 'No Classes For This Student'
                          : 'Select Course'}
                    </option>
                    {classOptionsForStudent.map(course => (
