      ...
      User.findByEmail(email, async (err, results) => {
        ...
        try {
          ...
          User.setResetToken(email, hashedToken, expiryDate, async (err, result) => {
            ...
            // Send password reset email (non-fatal - always return success for security)
            sendPasswordResetEmail(user.email, resetLink, user.name).catch((emailError) => {
              // Log error but don't fail the request (prevents user enumeration)
              console.error('Warning: Failed to send reset email to ' + user.email + ':', emailError.message);
              console.error('Make sure EMAIL_USER and EMAIL_PASS are configured in .env file');
            });
